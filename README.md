...
- **有栅栏的检索**：同时校验当前 index generation 与 intake serving eligibility；调用方必须显式选择 Layer A namespace，返回可回溯上下文，不生成最终答案。
...
| 本地 Turso Database 与 write-path / native-vector readiness | `已落地` | [`src/persistence/turso/`](src/persistence/turso/)；`/ready` 栅栏组件是 `write_path_ready`，不是 `concurrent_writes=true` |
...
| 测试 / 静态检查 | pytest `8.3.5`、pytest-asyncio `0.26.0`、Ruff `0.11.8` | unit/domain/integration/e2e/intake 分层；Ruff 已清零，全量 pytest 不是全绿 |
| 构建 | setuptools `>=80`、uv lockfile | 生成 wheel/sdist；wheel 含 migration SQL 与 LS-RAG JSON schema；运行还需要 checked-in `data/` 资源 |
...
[`api/`](api/) 定义唯一 FastAPI 应用、依赖注入、鉴权顺序和路由。`api.app:create_app()` 负责组装持久化、注册表、推理 facade、工作流 worker、检索、GC 与 retention，并安装 TrustedHost 与请求体上限中间件；`api.app:app` 是 ASGI 入口，`mkb` 是命令行入口。

### 3.2 合同层

[`src/contracts/`](src/contracts/) 存放公共 API、intake、workflow、inference、vector、storage、observability、persistence 和 LS-RAG 的版本化 Pydantic 模型。外部 UUID 仅接受 v4/v7，MKB 自己生成 UUIDv7；公共扩展只能进入显式 `payload_extra`。[`src/contracts/lsrag/`](src/contracts/lsrag/) 保存 layered content 与 quoted cuts 合同。

### 3.3 运行时与工作流

[`src/runtime/`](src/runtime/) 承担配置、健康检查、安全、HTTP 获取、推理调度、intake pipeline、Task 服务组合和 durable workflow engine。实现按 `task/`、`workflow/`、`inference/`、`intake/` 拆分；[`src/runtime/intake/generation_assemble.py`](src/runtime/intake/generation_assemble.py) 是系统写 g0 与锚点切片的组装器。[`src/workflows/`](src/workflows/) 保存当前声明式图以及供已冻结 Execution 恢复使用的历史 revision。
...
[`src/persistence/`](src/persistence/) 提供 migration、repository/access port、Turso 主路径和 pytest-only SQLite 实现。当前 migration 为 `001`–`017`。[`src/storage/`](src/storage/) 提供本地 CAS；对象内容和关系事实分离，数据库只保存逻辑 handle、digest 与引用。
...
[`tests/`](tests/) 按 unit、domain、integration、e2e、intake 分层。[`docs/baseline/domain-truth/`](docs/baseline/domain-truth/) 是设计真相层；`docs/closure/` 记录阶段收口（含 `0820-review/` 与 NS7/NS8），`docs/eval/` 记录实际运行评估，`docs/plan/` 只代表方案，不能被当成已实现事实。
...
│   └── internal/                  # /internal prompt 与运维观测 API
├── intake/
...
│   ├── persistence/
│   │   ├── migrations/            # 001–017 数据库 migration
│   │   └── turso/                 # pyturso 生产主路径
...
│   ├── prompts/                   # 经 hash 注册的 prompt 字节（g1 当前 active 为 v5）
...
documentation g1 的 structurize 默认走 quoted cuts：模型只交起止锚点，组装器写入恰好一块 `g0.body = clean`，再在 `clean` 上切片。超过约 10k 字符的 cuts 输入会按标题边界自适应分段。
...
确定性 text/JSON/HTML 清洗与有限 PDF text extraction 已接线。HTML 抽取会保留段落换行，而不是把 `<br>` 压成单个空格。OCR、Vision、browser rendering、doc-LLM、web-LLM 和 PDF-understanding 的合同、工作流或拒绝路径已经存在，但 `create_app()` 没有注入相应 runtime；因此这些 profile 不能描述为 live capability。
...
granularity 是闭集：`g0 → {0}`、`g1 → {0,1}`、`g2 → {0,1,2}`。flavor 只有在 `domain=documentation` 时合法；调用方可以给 prompt identity，但不能提交 prompt 正文、文件路径或自由 role。g1 v1–v4 字节仍保留在仓库中，供已冻结 Execution 按 hash 恢复，新 Task 解析到 v5。
...
live embed 的 key 随 Layer A 变化，形状为 `{model_key}|{model_version}|{adapter_kind}|{dimension}`。不要使用 `default`。
...
MKB 本身就是内部 backend/leaf worker，不是浏览器 BFF。业务接口和 operator 接口都要求内部 token，首选 `Authorization: Bearer <token>`，兼容 `X-MKB-Internal-Token`。鉴权在资源读取之前执行；operator 路由还要求 ASGI peer 为 loopback/private/internal 地址。`X-Forwarded-For` 只在 peer 属于 `MKB_TRUSTED_PROXY_CIDRS` 时才被采用；空 CIDR 永不复制 XFF。
...
`/ready` 的 REQUIRED 组件是：`schema_migration`、`registry_bootstrap`、`db_primary`、`write_path_ready`、`native_vector`、`object_root`、`inference_binding`、`obs_tables`、`sec_token_loaded`。业务 UoW 走 `BEGIN IMMEDIATE`；即使 `MKB_CONCURRENT_WRITES_REQUIRED=true`，admission 也不把 `concurrent_writes=true` 当作就绪条件。
...
| 层 | 覆盖 | 2026-08-21 @ `86037dd` 结果 |
|---|---|---|
| `tests/unit` | 合同、服务、engine、security、registry、persistence、NS5–NS8 原语 | 486 collected；全量中无失败 |
| `tests/domain` | 领域状态机和不变量 | 39 collected；全量中无失败 |
...
| README 离线 API smoke | 新 Turso 库上的 Team → Task → retrieval | ready `200`、Team/Task create `201`、Task `succeeded`；缺 namespace 为 `422`；带 stub key 为 `200 / disposition=ok / 3 results` |
| R4 live cells | 真实 prompt/inference 生成 | 4/4 未通过；既有 corpus retrieval 6/6 为 HTTP 200，不能据此宣称端到端 live |
| R6/R7 live cells | 系统 g0 + cuts + 10k 分段 | 代码与 preflight 已记录；业主发车仍 pending |
...
`uv build` 已在本次核对中成功生成标准 sdist 与 wheel，包范围为 `api*`、`src*`、`intake*`，并打包 `src/persistence/migrations/*.sql` 与 `src/contracts/lsrag/*.json`。运行还需要 checked-in `data/prompts`、`data/config`、`data/schemas`，因此单独复制 wheel 并不等于完整部署制品；部署必须显式挂载/打包这些资源并提供可写 DB/object 路径。

仓库当前没有容器、systemd、Kubernetes、CI/CD、反向代理或 hosting manifest，也没有 deploy 命令和生产 URL。部署方需自行负责进程守护、TLS、网络 ACL、可信代理 CIDR、secret 注入、数据卷、备份与恢复；完成这些工作前不能把本仓状态标为 production/live。
...
当前 HEAD 已形成一套边界清晰、证据优先的有状态 LS-RAG 内部工作器：合同、耐久执行、Turso/CAS、分层生成、发布栅栏、检索和审计面彼此衔接。NS5/NS6 把 cancel、CAS、XFF 和 publication fence 收成 fail-closed；NS7/NS8 把系统写 g0、quoted cuts 和中长文分段写入主链。离线 deterministic 配置适合本地开发与核心 unit/domain/integration 验证。

它还不是可对外宣称 production/live 的完整产品：全量测试未全绿，R4 真实推理链失败，R6/R7 live 仍待业主发车，部分来源 capability 未接线，部署和边缘 TLS 也不在仓库内。下一阶段最有价值的工作是先让 e2e/intake 合同跟上已落地的 namespace 与 HTML 换行，再完成真实推理验收，而不是扩大公开接口面。
...
| [`docs/closure/new-start/NS7-updates-before-0815-R6-closure.md`](docs/closure/new-start/NS7-updates-before-0815-R6-closure.md) / [`NS8-updates-before-0815-R7-closure.md`](docs/closure/new-start/NS8-updates-before-0815-R7-closure.md) | cuts / 10k 分段代码相收口；live 仍 pending |
...
| [`docs/code-review/`](docs/code-review/) | 外部/交叉 code review 记录，含 `0820-review/` |
