...
class IntakeCoreMixin:
    ...
    def __init__(
        ...
        *,
        ...
        pdf_parser: object | None = None,
        deterministic_ocr: object | None = None,
    ) -> None:
        ...
        self._deterministic_ocr = deterministic_ocr
    ...
    def _scatter_child_state(
        ...
        if any(not isinstance(member.get(key), str) or not member[key] for key in required_text):
            raise MkbError(
                "SCATTER_CHILD_MANIFEST_INVALID", "Scatter child manifest lacks an exact accepted member", 422
            )
