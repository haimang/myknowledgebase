...
class IntakeCleanPreflightMixin:
    ...
    async def _clean(
        ...
        if command.binding_state == "unsealed" and state.get("operation_mode") not in {"rebuild", "metadata_update"}:
        ...
        if command.process_key == "clean.ocr.local" and (
            not isinstance(media_type, str) or (not media_type.startswith("image/") and media_type != "application/pdf")
        ):
            raise MkbError("CLEAN_CAPABILITY_MISMATCH", "OCR clean capability requires image or PDF evidence", 409)
        if (
            command.process_key == "clean.extract.deterministic"
        ...
        result = await dispatch_clean(
            ...
            strategy=strategy,
            llm=llm,
            ocr=getattr(self, "_deterministic_ocr", None),
            prompt=prompt,
    ...
    async def _clean_registered_api(
        ...
        mapped = await dispatch_clean(
            ...
            definition_version=(
                state.get("api_definition_version") if isinstance(state.get("api_definition_version"), str) else None
            ),
        )
        ...
        next_state["collection_clean_digest"] = stable_digest([member["clean_digest"] for member in clean_members])
    ...
    async def _seal_registered_api_collection(
        ...
        if command.process_key != "intake.collection.seal":
            raise MkbError(
                "SCATTER_CAPABILITY_MISMATCH", "Collection sealing requires the registered seal capability", 409
            )
    ...
    def _validate_single_preflight_evidence(
        ...
        *,
        ...
        if (
            ...
            raise MkbError(
                "PREFLIGHT_EVIDENCE_INVALID", "Frozen acquisition representation evidence is incomplete", 422
            )
        if source_kind == "http_resource":
            if not all(
                isinstance(evidence.get(key), str) and evidence[key]
                for key in ("request_url_identity", "final_url_identity")
            ):
        ...
        expected_decode = (
            "intake.decode.pdf" if state.get("media_type") == "application/pdf" else "intake.decode.text_json_html"
        )
        ...
        root_digest = state.get("candidate_root_digest")
        if not isinstance(root_digest, str) or root_digest != stable_digest(
            {
    ...
    def _validate_rebuild_preflight_evidence(state: Mapping[str, Any]) -> list[dict[str, str]]:
        ...
        if (
            ...
            raise MkbError(
                "PREFLIGHT_BINDING_INVALID", "Frozen rebuild input is not bound to the accepted clean Artifact", 409
            )
        raw = state.get("raw_text")
        clean_text = state.get("clean_text")
        clean_digest = state.get("clean_digest")
