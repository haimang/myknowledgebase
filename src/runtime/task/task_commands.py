    ...
    async def retry(self, team_uuid: str, task_uuid: str, request: RetryRequest) -> dict[str, Any]:
        ...
        async with self.persistence.transaction() as tx:
            ...
            await self._insert_root_execution(
                ...
                s05_binding_digest=previous["domain_binding_digest"],
                actual_binding_digest=previous.get("actual_binding_digest"),
                actual_binding_state=str(previous.get("actual_binding_state") or "legacy_unverifiable"),
                seal_generation=int(previous.get("seal_generation") or 0),
                actual_selected_route_digest=previous.get("actual_selected_route_digest"),
                actual_clean_step_key=previous.get("actual_clean_step_key"),
                actual_clean_process_key=previous.get("actual_clean_process_key"),
                actual_clean_strategy=previous.get("actual_clean_strategy"),
