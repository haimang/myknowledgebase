...
| `S15-T050` | Timeline 投影字段：event_uuid、occurred_at、event_type、aggregate、severity、subject_*、summary、trace_uuid、task/execution/process uuid、payload_digest；**默认不**返回完整 payload_json 除非 debug 门控且仍脱敏。**NS4 `T-O-374`**：`timeline_by_task` 必须附带同 process 的 `generation_invocations`（status/stage_key/error_code/kind）与 `generation_stage_reports`（disposition/histogram 字段）。**禁止**把 `payload_extra.structure_reject` 当证据面。 | T-O-308/311 / T-O-374 |
