...
## 已完成并提交 (main 分支, 工作树干净, 全量 `python3 -m pytest tests/` 退出码 0 = **161 passed**, F1-F6a)
...
- **F6a 清洗器** ✅ 已收口 (`2dbceab`): CleanActionRegistry 分派(取代 if/else 硬选)+创建侧写 action_branch; htmlCrawl 真抓取(fetch_url 可注入/UA/超时/UrlFetchError)+html.parser 去标签保正文; chinatax_etl 真 ETL(fetch_api 可注入/解析结构化); browser/PDF/多 provider/scatter degraded(抛 DegradedActionError)。详见 `FF-F6a-closure.md`。
