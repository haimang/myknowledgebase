...
async def test_bootstrap_registers_four_roles_and_json_closed_sets(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert json_by_id["promptB.documentation.g1"].prompt_version in {"v4", "v5"}
        assert json_by_id["promptB.documentation.g1"].relative_path.endswith(("g1.v4.md", "g1.v5.md"))
...
async def test_bootstrap_retires_previous_active_g1_when_default_moves_to_v3(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert latest.prompt_version in {"v4", "v5"}
        assert latest.relative_path.endswith(("g1.v4.md", "g1.v5.md"))
