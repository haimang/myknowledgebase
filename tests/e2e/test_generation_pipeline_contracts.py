...
def test_generation_members_are_independent_and_vectorize_every_channel(tmp_path: Path) -> None:
    ...
    assert all(
        ...
        == by_type[
            "structure_validation_report"
            if artifact["artifact_type"].startswith("structure")
            or artifact["artifact_type"] == "retrieval_block_projection"
            else "construction_validation_report"
        ]["logical_handle"]
        for artifact in artifacts
    )
    assert {pointer["artifact_type"] for pointer in pointers} == expected_types
    assert {pointer["artifact_type"]: pointer["current_generation_artifact_uuid"] for pointer in pointers} == {
        artifact_type: artifact["generation_artifact_uuid"] for artifact_type, artifact in by_type.items()
    }

    structure_document = json.loads(
        _object_bytes(object_root, team_uuid, by_type["structure_document"]["logical_handle"])
    )
    projection = json.loads(
        _object_bytes(object_root, team_uuid, by_type["retrieval_block_projection"]["logical_handle"])
    )
    ...
    assert (
        construction["structure_generation_artifact_uuid"] == by_type["structure_document"]["generation_artifact_uuid"]
    )
    assert (
        construction["projection_generation_artifact_uuid"]
        == by_type["retrieval_block_projection"]["generation_artifact_uuid"]
    )
    ...
    assert all(
        vector["generation_artifact_uuid"] == by_type["dual_channel_projection"]["generation_artifact_uuid"]
    ...
    assert (
        vectorize_intent["construction_artifact_uuid"] == by_type["construction_document"]["generation_artifact_uuid"]
    )
    assert (
        vectorize_intent["dual_channel_artifact_uuid"] == by_type["dual_channel_projection"]["generation_artifact_uuid"]
    )
