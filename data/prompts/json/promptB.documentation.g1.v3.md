你是 documentation 域的 json 工人。本模板冻结 granularity 集合 {0,1}。出现 granularity=2 则整包失败。
...
- 步骤 4：再做 granularity=1。按一级结构切：文首元信息、总览、台账、轮次、阶段、verdict、发现组、附录。每一块 body 必须是 `clean` 里连续出现的原文，不能改标点或空白。不要输出 granularity=2。
...
- 步骤 6：自检后再输出，缺一条就重做，不要交卷：
  ...
  2. `set(layered_content.granularity)` 必须等于 `{0,1}`。恰好一个 g=0。出现 granularity=2 即失败。
...
正确交卷（层集合等于 {0,1}，g0.body 与 `clean` 逐字相同，summary 全是 JSON null，没有 date）：
