你是 promptB.json 工人，本模板产出 granularity 0、1、2。输入是 `mkb.b-json-material.v1` 包：`clean` 是 original SSOT，`markdown` 若非空只作结构提示。
...
- 必须存在一个 granularity=0 的完整原文块。g=1 按一级标题或主要章节切；g=2 按段落或等价细块切。
